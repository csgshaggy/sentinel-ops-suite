...
def run():
    try:
        out = subprocess.check_output(
            ["docker", "ps", "--format", "{{.ID}} {{.Ports}}"], text=True
        )
        ...
        running_backend = any(
            ":8000->" in c["ports"] or ":8000/" in c["ports"] for c in containers
        )

        status = running_backend
        details = (
            "Backend container running" if status else "No container exposing port 8000"
        )
