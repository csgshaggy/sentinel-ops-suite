...
def detect_missing_internal_modules(py_files):
    missing = []
    for f in py_files:
...
def find_misplaced_modules():
    ...
    for py_file in PROJECT_ROOT.rglob("*.py"):
        if "src/ssrf_console" in str(py_file):
            continue
        if py_file.name in {
            "project_doctor.py", "repair_imports.py",
            "auto_repair_structure.py", "fix_imports.py",
            "validate_imports.py"
        }:
            continue
...
def build_dep_graph(py_files):
    ...
    for f in py_files:
        ...
        graph[module_name] = parse_imports(f)
    return graph
...
def project_health(py_files):
    ...
    encoding_issues = [f for f in py_files if safe_read(f)[0] is None]
    if encoding_issues:
        print(f"[!] {len(encoding_issues)} encoding issues:")
        for f in encoding_issues:
            print(f"  - {f}")
    else:
        print("[OK] All files UTF-8 decodable.")

    # Missing modules
    header("MISSING INTERNAL MODULES")
    missing = detect_missing_internal_modules(py_files)
    if missing:
        for src, mod, path in missing:
            print(f"[!] {src} imports {mod} -> missing {path}")
    else:
        print("[OK] No missing internal modules.")
...
def main():
    ...
    header("STEP 4 — Full Project Health Dashboard")
